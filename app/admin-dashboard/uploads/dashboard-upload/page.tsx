...
export default function UploadPage() {
  ...
  const inputIdRef = useRef<string>(`dash-file-input-${Math.random().toString(36).slice(2)}`);
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-6 md:p-10">
          ...
          {files.length > 0 && (
            <button
              ...
              disabled={uploading}
              className={`mt-6 w-full text-white px-4 py-2 rounded ${uploading ? 'bg-green-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'}`}
            >
